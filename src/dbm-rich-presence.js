...
function overrideFunctions() {
 ...
 DBM.onCommandClick = function(index) {  
   try {
     ...
     const details = enableCmdNames ? `${index ? `${type}: ` : ' '}${index ? (getName(section, index) || `New ${type}`) : `No Selection`}` : `Editing ${section}`;     
 ...
 const noSaveAndClose = DBM.noSaveAndClose;
 DBM.noSaveAndClose = function() {
   alert('before')
   try {
     rpcOptions.state = `No Project Opened`;
     rpc.setActivity(rpcOptions); 
    ...
    alert('after')
    noSaveAndClose.apply(this, arguments);
  }
}
